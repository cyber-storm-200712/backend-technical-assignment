...
const connection = createConnection(process.env.DB_HOST, process.env.USER, process.env.PASSWORD, process.env.TEST_DB_NAME);
...
connection.connect((err) => {
    if (err && err.code == Erros.ER_BAD_DB_ERROR) {
        console.warn(`DB "${process.env.TEST_DB_NAME}" is not exist.`);
        process.exit(1);
    }
})
...
const createTables = async () => {
    connection.query(Queries.SQL_CREATE_STATIONS_TABLE_TEST, (err, res) => {
        if (err) console.warn(err);
        if (res) console.warn(`Table "stations_test" was created.`);
    })

    connection.query(Queries.SQL_CREATE_WEATHERS_TABLE_TEST, (err, res) => {
        if (err) console.warn(err);
        if (res) console.warn(`Table "weathers_test" was created.`);
    })
}
...
const addDataToDB = async () => {
    ...
    indegoRows.map(item => {
        connection.query(Queries.SQL_INSERT_STATIONS_DATA_TEST, item, (err) => {
            if (err) console.log(err);
        })
    })

    connection.query(Queries.SQL_INSERT_WEATHERS_DATA_TEST, weatherRow, (err) => {
        if (err) console.log(err);
    })
}
...
const fetchStationDataFromDBById = async (timestamp, kioskId) => {
    return new Promise((resolve, reject) => {
        connection.query(`SELECT * FROM stations_test WHERE YEAR(at) = ${new Date(timestamp).getFullYear()} AND MONTH(at) = ${new Date(timestamp).getMonth() + 1} AND DAY(at) = ${new Date(timestamp).getDate()} AND HOUR(at) = ${new Date(timestamp).getHours()} AND kioskId = ${kioskId}`, (err, res) => {
            if (err) return reject(err);
            if (res) return resolve(res);
        })
    })
...
const fetchWeathersDataFromDBByDateTime = async (timestamp) => {
    return new Promise((resolve, reject) => {
        connection.query(`SELECT * FROM weathers_test WHERE YEAR(at) = ${new Date(timestamp).getFullYear()} AND MONTH(at) = ${new Date(timestamp).getMonth() + 1} AND DAY(at) = ${new Date(timestamp).getDate()} AND HOUR(at) = ${new Date(timestamp).getHours()}`, (err, res) => {
            if (err) return reject(err);
            if (res) return resolve(res);
        })
    })
...
const clearAllDataFromDB = async () => {
    connection.query(Queries.SQL_CLEAR_STATIONS_DATA, (err, res) => {
        if (err) console.warn(err);
        if (res) console.warn(`Table "stations" was cleared.`);
    })

    connection.query(Queries.SQL_CLEAR_WEATHERS_DATA, (err, res) => {
        if (err) console.warn(err);
        if (res) console.warn(`Table "weathers" was cleared.`);
    })
}
